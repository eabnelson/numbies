...
# Find the built app
APP_PATH=$(find ~/Library/Developer/Xcode/DerivedData/numbiesxyz-*/Build/Products/Debug-iphoneos -name "Numbies.app" -type d 2>/dev/null | head -1)
...
echo -e "${YELLOW}Make sure 'bun dev' is running in another terminal.${NC}"
echo -e "${YELLOW}Tip: Shake device for dev menu${NC}"
